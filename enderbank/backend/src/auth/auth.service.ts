...
export class AuthService {
	...
	private static isPasswordValidForUser(user: User, password: string) {
		return user.passwordHash === AuthService.hashPassword(password, user.passwordSalt);
	}
	...
	public async login(request: LoginRequest): Promise<AuthorizingResponse> {
		...
		if (!existingUser) {
			throw new HttpException("User with given username dosnt't exists!", HttpStatus.BAD_REQUEST);
		}

		if (!AuthService.isPasswordValidForUser(existingUser, request.password)) {
	...
	public async register(request: RegisterRequest): Promise<AuthorizingResponse> {
		const existingUser = await this._user.findOne({userName: request.userName}).exec();
		if (existingUser) {
